...
unsigned int _tstringW::Replace(const _tstringW& strOld, const _tstringW& strNew)  
{
  ...
  //  }
	...
	size_t pos = 0 ;
	pos = Find(strOld, (unsigned int)pos) ;
	while (pos != std::string::npos)
	{
		replace(pos,strOld.length(), strNew) ;
		pos = Find(strOld, (unsigned int)(pos+strNew.length())) ; //search for the remaining content
		++ nCount ;
	} 
...
{
	if (this->empty()) // fix bug : when str is empty, conver faild.
	{
		return "";
	}
